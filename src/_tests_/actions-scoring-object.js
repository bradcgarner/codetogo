...
describe('findIndexByPositions', () => {
  ...
  it('should move 1 position when 0 positions passed', () => {
    ...
    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->${indexAfter}...*...${indexBefore})`, 
      illegal: {
  ...
  it('should move 1 position when 1 positions passed', () => {
    const questions = [
      {indexNext: 1},
      {indexNext: 2},
      {indexNext: 3},
  ...
  it('should move 2 positions when 2 positions passed', () => {
    ...
    const indexAfter = 3;
    const indexBefore = 4;
    const newPositions = 2;

    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->2->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect],
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('should move 3 positions when 3 positions passed', () => {
    ...
    const indexAfter = 4;
    const indexBefore = 5;
    const newPositions = 3;

    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->2->3->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect],
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('should move 4 positions when 4 positions passed', () => {
    ...
    const indexAfter = 5;
    const indexBefore = 6;
    const newPositions = 4;

    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->2->3->4->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect],
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('should move past illegal positions (5 passed)', () => {
    const questions = [
      {indexNext: 1},
      {indexNext: 2},
      {indexNext: 3},
    ...
    const indexAfter = 1;
    const indexBefore = 2;
    const newPositions = 8;

    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->2->3->4->5->6->7->0->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect],
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('scramble: should skip illegal positions 1', () => {
    const questions = [
      {indexNext: 2}, // 0
      {indexNext: 3}, // 1
      {indexNext: 6}, // 2
    ...
    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>6->7->4->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect],
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('scramble: should skip illegal positions 2', () => {
    ...
    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>6->7->4->5->1->3->0->2->${indexAfter}...*...${indexBefore})`, 
      illegal: {
        indexInsertAfter: [indexCurrent, indexRedirect], 
        indexInsertBefore: [indexCurrent, indexRedirect],
      }
  ...
  it('scramble: should skip illegal positions 2', () => {
    ...
    const expectedResult  = {
      indexAfter, indexBefore, positions: newPositions,
      label: `${newPositions} positions (${indexCurrent}-->>1->3->0->2->6->7->4->5->${indexAfter}...*...${indexBefore})`, 
      illegal: {
